...
func (s *websocketService) AddToGroup(client *models.Client, groupID string) {
    ...
    if actualClient, exists := s.clients[client.Username]; exists {
        ...
        s.groups[groupID][client.Username] = client
...
func (s *websocketService) KickFromGroup(username string, groupID string) {
    ...
    if s.groups[groupID] != nil {
        delete(s.groups[groupID], username)
    }
...
func (s *websocketService) readPump(client *models.Client) {
    defer func() {
        ...
        for groupID := range s.groups {
            delete(s.groups[groupID], client.Username)
        }
        s.mutex.Unlock()
        client.Conn.Close()
...
func (s *websocketService) sendMessage(client *models.Client, message []byte) {
    ...
    case <-time.After(time.Second):
        log.Printf("Failed to send to client %s, closing connection", client.Username)
        close(client.Send)
        s.mutex.Lock()
        delete(s.clients, client.Username)
        for gid := range s.groups {
            delete(s.groups[gid], client.Username)
        }
        s.mutex.Unlock()
    }
}
...
func (s *websocketService) handleChatMessage(msg *models.Message) {
    ...
    if msg.Receiver != "" {
        ...
        if sender, exists := s.clients[msg.Sender]; exists {
